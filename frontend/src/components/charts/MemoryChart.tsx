/**
 * MemoryChart — Cinematic real-time memory usage area chart.
 */
import {
...
interface MemoryChartProps { history: SystemMetricsSnapshot[] }

export function MemoryChart({ history }: MemoryChartProps) {
  const data = history.map((s) => ({
    time: fmt.time(s.timestamp),
    memory: +s.memory.percent.toFixed(1),
  }))
      ...
      <AreaChart data={data} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
        <defs>
          <linearGradient id="gradMemory" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor={chartColors.memory} stopOpacity={0.35} />
            <stop offset="100%" stopColor={chartColors.memory} stopOpacity={0.02} />
          </linearGradient>
        </defs>
        ...
        <Area type="monotone" dataKey="memory" stroke={chartColors.memory} strokeWidth={2} fill="url(#gradMemory)"
          dot={false} activeDot={{ r: 5, fill: chartColors.memory, stroke: 'rgba(10,10,32,0.8)', strokeWidth: 2, style: { filter: `drop-shadow(0 0 6px ${chartColors.memory})` } }}
          isAnimationActive animationDuration={600} style={{ filter: `drop-shadow(0 0 4px ${glowColor(chartColors.memory)})` }}
        />
      </AreaChart>
    </ResponsiveContainer>
  )
